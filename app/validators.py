...
def scale_constraint(field_name):
    ...
    return CheckConstraint(
        f"{field_name} >= {MIN_SCALE_VALUE} AND " f"{field_name} <= {MAX_SCALE_VALUE}"
    )
...
def housing_constraint():
    ...
    return CheckConstraint(
        f"housing >= {MIN_HOUSING_LEVEL} AND " f"housing <= {MAX_HOUSING_LEVEL}"
    )
...
def success_rate_constraint():
    ...
    return CheckConstraint(
        f"success_rate >= {MIN_SUCCESS_RATE} AND " f"success_rate <= {MAX_SUCCESS_RATE}"
    )
